...
<div id="postbox">
		<ul id="post-types">
			<li><a id="post"<?php if ( $_GET['p'] == 'post'  || !isset($_GET['p']) ) : ?> class="selected"<?php endif; ?> href="<?php echo site_url( '?p=post' ) ?>" title="<?php _e( 'New ticket', 'p2' ) ?>"><?php _e( 'New ticket', 'p2' ) ?></a></li>
		</ul>
		...
		<div class="inputarea">
			...
			<form id="new_post" name="new_post" method="post" action="<?php echo site_url(); ?>/">
				<?php if ( 'status' == p2_get_posting_type() ) : ?>
				<label for="posttext">
					<?php p2_user_prompt() ?>
				</label>
				<?php endif; ?>

				<div id="postbox-type-post" class="post-input <?php if ( 'post' == p2_get_posting_type() || '' == p2_get_posting_type() ) echo ' selected'; ?>">
