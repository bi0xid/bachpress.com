...
function wpet_admin_header(){
	...
	echo PHP_EOL.'<link rel="stylesheet" type="text/css" media="screen" href="'.get_settings('siteurl').'/wp-content/plugins/wp-easy-todo/wp_easy_todo.css" />'.PHP_EOL;
	echo '<script type="text/javascript" src="'.get_settings('siteurl').'/wp-content/plugins/wp-easy-todo/js/jquery-1.3.2.js"></script>'.PHP_EOL;
	echo '<script type="text/javascript" src="'.get_settings('siteurl').'/wp-content/plugins/wp-easy-todo/js/jquery-color.js"></script>'.PHP_EOL;
    ?>
...
$(document).ready(function(){
	...
	$('a.wpet_ready').click(function(e) {
		if(confirm('Are your sure this item is already done?') == true){
			...
			$.ajax({
				type: 'get',
				url: '<?php echo get_settings('siteurl');?>/wp-content/plugins/wp-easy-todo/wp_easy_todo_ajax.php',
...
function wpet_installation(){
	...
	if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
		$sql = "CREATE TABLE `db017886_000`.`".$table_name."` (
